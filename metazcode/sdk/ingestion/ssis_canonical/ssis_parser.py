...
class CanonicalSsisParser:
    ...
    def __init__(
        ...
        self.target_platforms = self._parse_target_platforms(target_platforms or ["sql_server", "postgresql", "databricks", "snowflake", "fabric"])
    ...
    def _parse_package(
        ...
        # Parse tasks recursively (handles Sequence Containers, ForEachLoop, ForLoop)
        executables_container = root.find("DTS:Executables", self.ns_map)
        if executables_container is None:
            yield nodes, edges
            return

        self._parse_executables_recursive(
            executables_container=executables_container,
        ...
        # Parse precedence constraints (control flow) - includes nested containers
        self._parse_precedence_constraints(root, pipeline_id, edges)

        yield nodes, edges

    def _parse_executables_recursive(
        self,
        executables_container: etree._Element,
        parent_id: str,
        nodes: List[Node],
        edges: List[Edge],
        ...
        """
        Recursively parse all executables including those nested inside containers.

        This method handles:
        - Sequence Containers (STOCK:SEQUENCE)
        - For Loop Containers (STOCK:FORLOOP)
        - ForEach Loop Containers (STOCK:FOREACHLOOP)
        - Data Flow Tasks (Microsoft.Pipeline)
        - Execute SQL Tasks
        - Script Tasks
        - And any other task types

        Args:
            executables_container: The DTS:Executables XML element containing tasks
            parent_id: The ID of the parent node (pipeline or container)
            nodes: List to append nodes to
            edges: List to append edges to
            connection_id_map: Mapping of connection GUIDs to node IDs
            param_var_id_map: Mapping of parameter/variable names to node IDs
            file_path: Path to the source file for traceability
            package_name: Name of the package for traceability
        """
        for task_xml in executables_container.findall("DTS:Executable", self.ns_map):
            ...
            executable_type = task_xml.get(f"{{{self.ns_map['DTS']}}}ExecutableType")

            if not task_name or not executable_type:
                continue

            task_id = f"{parent_id}:operation:{task_name}"

            # Get operation subtype
            operation_subtype = self._categorize_operation_subtype(executable_type)

            # Determine if this is a container type
            is_container = executable_type in ["STOCK:SEQUENCE", "STOCK:FORLOOP", "STOCK:FOREACHLOOP"]

            # Create operation node with enhanced traceability
            operation_properties = {
                "native_type": executable_type,
            ...
            if object_data_xml is not None:
                ...
                if task_type_element is not None:
                    task_type_tag = task_type_element.tag

                    if "pipeline" in task_type_tag.lower():
                        # Data Flow Task - parse components
                        pipeline_xml = task_type_element
                        components_xml = pipeline_xml.find("components")
                        if components_xml is not None:
                            for component_xml in components_xml.findall("component"):
                                ...
                                # Apply pending SQL semantics if collected during component parsing
                    ...
                    elif "SqlTaskData" in task_type_tag:
                    ...
                    elif "ScriptTaskData" in task_type_tag or "ScriptProject" in task_type_tag:
                    ...
                    elif "ForEachLoop" in task_type_tag or "FOREACHLOOP" in task_type_tag:
                        # Parse Foreach Loop Container configuration
                        ...
                        logger.debug(
                            f"DEBUG: Unhandled task type: '{task_type_tag}' for task: '{task_name}'"
                        )

                    # Apply file path parameter resolution to control flow tasks
            ...
            # Parse Event Handlers for this task
            ...
            # CRITICAL: Recursively parse nested executables inside containers
            # This handles Sequence Containers, ForEachLoop, ForLoop, etc.
            nested_executables = task_xml.find("DTS:Executables", self.ns_map)
            if nested_executables is not None:
                logger.debug(f"Recursing into container '{task_name}' ({executable_type})")
                self._parse_executables_recursive(
                    executables_container=nested_executables,
                    parent_id=task_id,
                    nodes=nodes,
                    edges=edges,
                    connection_id_map=connection_id_map,
    ...
    def _parse_connection_managers(
        ...
        for conn_xml in connections_container.findall(
            ...
            conn_guid = conn_xml.get(f"{{{self.ns_map['DTS']}}}DTSID")
            ...
            # Map both GUID and name to node ID for flexible lookup
            id_map[conn_guid] = conn_id
            id_map[conn_name] = conn_id  # Also map by name for connectionManagerID references
    ...
    def _parse_oledb_component(
        ...
        if connections_tag is not None:
            ...
            if connection_tag is not None:
                ...
                if conn_mgr_id_attr:
                    # connectionManagerID can be either:
                    # 1. GUID format: "{GUID}:refId"
                    # 2. Path format: "Package.ConnectionManagers[ConnectionName]"
                    if "ConnectionManagers[" in conn_mgr_id_attr:
                        # Extract connection name from path
                        conn_key = conn_mgr_id_attr.split("[")[1].rstrip("]")
                    else:
                        # Extract GUID
                        conn_key = conn_mgr_id_attr.split(":")[0].strip("{}")

        if conn_key and conn_key in connection_id_map:
            edges.append(
                Edge(
                    source_id=task_id,
                    target_id=connection_id_map[conn_key],
        ...
        if properties_tag is not None:
            ...
            sql_text = sql_command_prop.text if sql_command_prop is not None else None

            if sql_text:
                sql_upper = sql_text.upper().strip()

                # Handle stored procedure calls (EXEC/EXECUTE)
                if sql_upper.startswith("EXEC") or "EXEC " in sql_upper:
                    self._parse_stored_procedure_call(
                        sql_text=sql_text,
                        task_id=task_id,
                        is_source=is_source,
                        nodes=nodes,
                ...
                # Handle SELECT statements
                    ...
                    if sql_semantics and sql_semantics.tables:
                        ...
                        for table_ref in sql_semantics.tables:
                            ...
                            table_name_clean = self._clean_table_name(sql_table_name)
                            ...
                            if not any(n.node_id == table_id for n in nodes):
                                ...
                                if conn_key and conn_key in self.connections_context:
                                    conn_info = self.connections_context[conn_key]
                                    ...
                                    if connection_string:
                                        ...
                                        table_properties["connection_id"] = conn_key
    ...
    def _parse_stored_procedure_call(
        self,
        sql_text: str,
        task_id: str,
        is_source: bool,
        ...
        """
        Parse stored procedure calls (EXEC/EXECUTE) and create procedure nodes with lineage edges.

        Handles patterns like:
        - EXEC Integration.GetCityUpdates ?, ?
        - EXECUTE dbo.MigrateStagedData
        - EXEC [Schema].[ProcedureName] @param1, @param2

        Also extracts WITH RESULT SETS metadata when available.

        Args:
            sql_text: The SQL text containing the EXEC statement
            task_id: The parent task ID
            is_source: True if this is an OLE DB Source (reads), False for Destination (writes)
            nodes: List to append nodes to
            edges: List to append edges to
            connection_id_map: Mapping of connection GUIDs to node IDs
            conn_key: Connection key for this component
            file_path: Source file path for traceability
            component_name: Name of the component
        """
        import re

        # Extract procedure name from EXEC statement
        # Patterns: EXEC Schema.Proc, EXEC [Schema].[Proc], EXECUTE dbo.Proc
        exec_pattern = r'(?:EXEC(?:UTE)?)\s+(\[?[\w]+\]?\.?\[?[\w]+\]?)'
        match = re.search(exec_pattern, sql_text, re.IGNORECASE)

        if not match:
            logger.debug(f"Could not extract procedure name from: {sql_text[:100]}")
            return

        proc_name_raw = match.group(1)
        proc_name = self._clean_table_name(proc_name_raw)  # Reuse table name cleaner for brackets

        # Extract WITH RESULT SETS columns if present
        result_set_columns = self._extract_result_set_columns(sql_text)

        # Extract procedure parameters (?, @param patterns)
        proc_parameters = self._extract_procedure_parameters(sql_text)

        # Create procedure node
        proc_id = f"procedure:{proc_name}"

        if not any(n.node_id == proc_id for n in nodes):
            proc_properties = {
                "technology": "SSIS",
                "procedure_name": proc_name,
                "asset_type": "STORED_PROCEDURE",
                "sql_text": sql_text[:500] if len(sql_text) > 500 else sql_text,  # Truncate for large SQL
                **SourceContext.create_node_traceability(
                    source_file_path=file_path,
                    source_file_type="dtsx",
                    xml_path=f"//property[@name='SqlCommand'][contains(text(), '{proc_name}')]",
                    parent_package=os.path.basename(file_path).replace('.dtsx', '')
                )
            }

            # Add result set columns if extracted (this is the procedure's OUTPUT schema)
            if result_set_columns:
                proc_properties["result_set_columns"] = result_set_columns
                proc_properties["output_column_count"] = len(result_set_columns)

            # Add procedure parameters if extracted (this is the procedure's INPUT)
            if proc_parameters:
                proc_properties["parameters"] = proc_parameters
                proc_properties["parameter_count"] = len(proc_parameters)

            # Add connection context if available
            if conn_key and conn_key in self.connections_context:
                conn_info = self.connections_context[conn_key]
                proc_properties["connection_id"] = conn_key
                proc_properties["database"] = conn_info.get("database")
                proc_properties["server"] = conn_info.get("server")
            ...
            logger.debug(f"Created stored procedure node: {proc_id} with {len(result_set_columns)} result columns")

        # Create edge - for sources we READ FROM the procedure, for destinations we might WRITE TO
        # Most EXEC in Data Flows are sources (data extraction)
        relation = EdgeType.READS_FROM if is_source else EdgeType.WRITES_TO
        ...
        edges.append(Edge(
            ...
            properties=SourceContext.create_edge_traceability(
                ...
                xml_location="//property[@name='SqlCommand']",
                context_info={
                    "sql_statement": sql_text[:200] if len(sql_text) > 200 else sql_text,
                    "component_type": "OLE DB Source" if is_source else "OLE DB Destination",
                    "component_name": component_name,
                    "procedure_name": proc_name,
                },
                confidence_level="high"
        ...
        logger.debug(f"Created {relation.value} edge: {task_id} -> {proc_id}")

    def _extract_result_set_columns(self, sql_text: str) -> List[Dict[str, Any]]:
        """
        Extract column definitions from WITH RESULT SETS clause.
        ...
        Returns:
            List of column dictionaries with name, data_type, and optional length
        """
        import re

        columns = []

        # Find WITH RESULT SETS block
        result_sets_match = re.search(
            r'WITH\s+RESULT\s+SETS\s*\(\s*\(\s*(.*?)\s*\)\s*\)',
            sql_text,
            re.IGNORECASE | re.DOTALL
        )

        if not result_sets_match:
            return columns

        columns_text = result_sets_match.group(1)

        # Parse individual column definitions
        # Pattern matches: [Column Name] datatype(length) or ColumnName datatype
        column_pattern = r'\[?([^\]\[,]+)\]?\s+([\w]+)(?:\s*\(\s*(\d+)(?:\s*,\s*(\d+))?\s*\))?'
        ...
        for match in re.finditer(column_pattern, columns_text):
            ...
            # Add type mapping if enabled
            if self.enable_type_mapping and self.type_mapper:
                # Map SQL Server type to canonical type
                type_properties = self.type_mapper.enrich_column_properties(
                    ssis_type=data_type,
                    length=length,
                    target_platforms=self.target_platforms
                )
                column_info.update(type_properties)

            columns.append(column_info)

        return columns

    def _extract_procedure_parameters(self, sql_text: str) -> List[Dict[str, str]]:
        """
        Extract procedure parameters from EXEC statement.

        Handles patterns like:
        - EXEC proc ?, ?  (positional parameters)
        - EXEC proc @Param1, @Param2  (named parameters)
        - EXEC proc @Param1 = ?, @Param2 = ?  (named with placeholders)

        Returns:
            List of parameter dictionaries with name and direction
        """
        import re

        parameters = []

        # Extract the parameter portion after procedure name
        exec_match = re.search(
            r'EXEC(?:UTE)?\s+[\w\.\[\]]+\s+(.*?)(?:WITH\s+RESULT|;|$)',
            sql_text,
            re.IGNORECASE | re.DOTALL
        )
        ...
        params_text = exec_match.group(1).strip()

        # Count positional parameters (?)
        positional_count = params_text.count('?')

        # Find named parameters (@ParamName)
        named_params = re.findall(r'@(\w+)', params_text)
        ...
        if named_params:
            for i, param_name in enumerate(named_params):
                parameters.append({
                    "name": f"@{param_name}",
                    "position": i + 1,
                    "direction": "Input"  # Most common case
                })
    ...
    def _parse_execute_sql_task(
        ...
        if sql_statement:
            ...
            # Handle EXEC/EXECUTE stored procedure calls
            if sql_upper.startswith("EXEC") or "EXEC " in sql_upper:
                self._parse_stored_procedure_call(
                    sql_text=sql_statement,
                    task_id=task_id,
                    is_source=False,  # Execute SQL Tasks typically perform actions (writes/calls)
                    nodes=nodes,
                    edges=edges,
                    connection_id_map=connection_id_map,
                    conn_key=connection_ref,
                    file_path=file_path,
                    component_name=task_id.split(":")[-1],  # Use task name as component name
                )
            else:
                        ...
                        if connection_tag is not None:
                            ...
                            if conn_mgr_id:
                                ...
                                if conn_guid in connection_id_map:
                                    conn_ref = connection_id_map[conn_key]
    ...
    def _clean_table_name(self, table_name: str) -> str:
        """
        Clean table name by removing SQL Server bracket notation.
        ...
        Args:
            table_name: Raw table name potentially with brackets

        Returns:
            Cleaned table name without brackets
        """
        if not table_name:
            return table_name
