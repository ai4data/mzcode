"""
Post-Processor for Informatica Refactored Parser
...
2. Cross-workflow dependency analysis
3. Execution priority assignment
4. LLM-powered node enrichment
5. LLM-powered edge enrichment
6. Column-level lineage tracing (future)
"""
...
class InformaticaPostProcessor:
    """
    Orchestrates post-processing pipeline for Informatica parsed graphs.

    Pipeline stages:
    1. Graph Cleaning - De-duplicate and normalize nodes/edges
    2. Cross-Workflow Analysis - Identify dependencies and execution order
    3. Node Enrichment - Add LLM summaries to workflows, mappings, operations
    ...
    def __init__(
        ...
        Args:
            ...
            enable_cross_workflow_analysis: Whether to run cross-workflow analysis
        ...
        self.enable_cross_workflow_analysis = enable_cross_workflow_analysis
        self.enable_llm_enrichment = enable_llm_enrichment
        self.skip_enriched = skip_enriched

        # Initialize components
        self.graph_cleaner = InformaticaGraphCleaner()

        if self.enable_cross_workflow_analysis:
            self.cross_workflow_analyzer = CrossPackageAnalyzer(graph_client)
    ...
    def process(self, nodes: List[Node], edges: List[Edge]) -> Dict[str, Any]:
        ...
        logger.info("="*70)
        logger.info("STARTING INFORMATICA POST-PROCESSING PIPELINE")
        logger.info("="*70)
        ...
        # Stage 2: Cross-Workflow Analysis
        if self.enable_cross_workflow_analysis:
            logger.info("\n[Stage 2/4] Cross-Workflow Dependency Analysis...")
            try:
                cross_workflow_results = self.cross_workflow_analyzer.analyze()
                self.stats["cross_workflow_stats"] = cross_workflow_results
            except Exception as e:
                logger.error(f"Cross-workflow analysis failed: {e}", exc_info=True)
                self.stats["cross_workflow_stats"] = {"error": str(e)}
        else:
            logger.info("\n[Stage 2/4] Cross-Workflow Analysis (SKIPPED)")
        ...
        if self.enable_cross_workflow_analysis:
            cross_stats = self.stats.get("cross_workflow_stats", {})
            logger.info(f"Cross-workflow edges added: {cross_stats.get('cross_package_edges_added', 0)}")
    ...
    def _update_graph_with_cleaned_data(self, nodes: List[Node], edges: List[Edge]):
        ...
        try:
            ...
            for node in nodes:
                ...
                # Extract name from properties to avoid conflict with keyword arg
                props = node.properties.copy()
                node_name = props.pop('name', node.name)
            ...
            for edge in edges:
                ...
                # Extract relation from properties to avoid conflict with keyword arg
    ...
    def _get_enrichable_nodes(self, nodes: List[Node]) -> List[Node]:
        ...
        Enrichable nodes:
        - OPERATION nodes (transformations, tasks)
        - PIPELINE nodes (workflows, mappings)
        """
