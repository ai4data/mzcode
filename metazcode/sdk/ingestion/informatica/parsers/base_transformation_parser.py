...
from .column_lineage_builder import ColumnLineageBuilder, JoinerLineageBuilder, LookupLineageBuilder
from .error_handling_builder import ErrorHandlingBuilder
...
class BaseTransformationParser(ABC):
    ...
    def __init__(self, context: InformaticaParsingContext, builder: InformaticaGraphBuilder):
        ...
        # Initialize column lineage builder (Enhancement #1)
        self.lineage_builder = self._get_lineage_builder()

        # Initialize error handling builder (Enhancement #5)
    ...
    def parse(
        ...
        # Step 3.5: Build column lineage (Enhancement #1)
        column_lineage = self.lineage_builder.build_lineage(
        ...
        # Step 3.6: Build error handling metadata (Enhancement #5)
    ...
    def _get_lineage_builder(self) -> ColumnLineageBuilder:
        """
        Get the appropriate lineage builder for this transformation type.

        Returns:
            Specialized lineage builder based on transformation type
        """
        transformation_type = self.get_transformation_type().lower()

        if "joiner" in transformation_type:
            return JoinerLineageBuilder()
        elif "lookup" in transformation_type:
            return LookupLineageBuilder()
        else:
